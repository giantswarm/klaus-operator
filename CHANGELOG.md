...
- Replace the pre-flight `Ping` liveness probe on cached agent MCP sessions with transport-error eviction. mcp-go v1.0.0 deprecates `Client.Ping`: protocol version 2026-07-28 removed the ping RPC (SEP-2575) and, on a modern connection, `Ping` returns `nil` without sending anything -- so the probe no longer proved that the agent behind a cached session was reachable. The outcome of the tool call itself is now the liveness signal: a failure wrapped in `*transport.Error` evicts the cached session, while a JSON-RPC error response leaves it in place instead of churning a healthy session whenever a caller passes bad arguments. `status` and `result` retry once on a rebuilt session, which keeps recovery from an agent restart transparent as the probe used to; `prompt` does not, because a transport error cannot distinguish a request that was never delivered from one whose response was lost, and re-sending it would make the agent act on the same message twice.
